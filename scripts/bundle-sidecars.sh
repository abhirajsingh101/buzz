...
if [[ "$TARGET" != *windows* ]]; then
    SIDECARS+=(buzz-backend-kubernetes buzz-backend-super)
    BUILD_HINT="cargo build --release -p buzz-acp -p buzz-agent -p buzz-backend-kubernetes -p buzz-backend-super -p buzz-dev-mcp -p git-credential-nostr -p buzz-cli"
else
